        ...
        <table class="table table-bordered">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td class="text-sm">
                        @if($proposal->status_disposisi == 'Menunggu Approval Perpus')
                    ...
                    <td class="d-flex">
                        ...
                        @if($proposal->perlu_sk && $proposal->tujuan_disposisi == $proposal->pihak_pembuat_sk)
                            ...
                            <form id="upload-sk-form-{{ $proposal->id }}" action="{{ route('plt.proposal.upload-sk', $proposal->id) }}" method="POST" enctype="multipart/form-data" style="display: none;">
                                @csrf
                    ...
                    <div class="modal-dialog" role="document">
                        <div class="modal-content">
                            ...
                            <div class="modal-body">
                                <form method="POST" action="{{ route('perpus.disposisi.updateDisposisi', $proposal->id) }}">
                                    @csrf
                                    @method('PUT')
                                    ...
                                    <div class="form-group">
                                        ...
                                        <select id="disposisi" class="form-control" name="disposisi">
                                            @foreach(json_decode($proposal->pihak_ttd, true) as $item)
                                                        <option value="{{ $item === 'Perpus' ? 'Staff TU' : $item }}">
                                                            {{ $item === 'Perpus' ? '🟢 Kembalikan ke Staff TU (Selesai)' : $item }}
                                                        </option>
                                            @endforeach
                                        </select>
                                    </div>
                ...
                <div class="modal fade" id="rejectModal{{ $proposal->id }}" tabindex="-1" role="dialog" aria-labelledby="rejectModalLabel" aria-hidden="true">
                    <div class="modal-dialog" role="document">
                        <div class="modal-content">
                            ...
                            <div class="modal-body">
                                <form method="POST" action="{{ route('perpus.disposisi.updateReject', $proposal->id) }}">
                                    @csrf
                                    @method('PUT')
