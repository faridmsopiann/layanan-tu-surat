...
<div class="container pb-4 mb-4" style="font-family: 'Roboto', sans-serif;">
    ...
    @if($arsipProposals->isEmpty())
        <form method="GET" action="{{ route('keuangan.arsip.index') }}" class="p-2 rounded shadow-sm bg-info">
            ...
            <a href="{{ route('keuangan.arsip.index') }}" class="btn btn-secondary btn-sm mr-1">
                <i class="fas fa-undo-alt"></i>
            </a>
        </form>
        <p class="text-center mt-4">Tidak ada proposal.</p>
    @else
        <form method="GET" action="{{ route('perpus.arsip.index') }}" class="p-2 rounded shadow-sm bg-info">
            <input type="text" name="kode_pengajuan" placeholder="Kode Pengajuan" value="{{ request('kode_pengajuan') }}" class="form-control form-control-sm d-inline w-auto mr-1 mb-1" aria-label="Cari Kode Pengajuan">
            <input type="date" name="tanggal_surat" value="{{ request('tanggal_surat') }}" class="form-control form-control-sm d-inline w-auto mr-1 mb-1" aria-label="Cari Tanggal Surat">
            <input type="text" name="asal_surat" placeholder="Asal Surat" value="{{ request('asal_surat') }}" class="form-control form-control-sm d-inline w-auto mr-1 mb-1" aria-label="Cari Asal Surat">
            <input type="text" name="perihal" placeholder="Perihal" value="{{ request('perihal') }}" class="form-control form-control-sm d-inline w-auto mr-1 mb-1" aria-label="Cari Perihal">
                    ...
                    @foreach($arsipProposals as $key => $proposal)
                        <tr class="align-middle" style="transition: background-color 0.3s;">
                            ...
                            <td class="text-sm">
                                @if ($proposal->soft_file)
                                    ...
                                    @elseif (count($files) > 1)
                                        <div class="mt-0">
                                            <a href="{{ route('keuangan.proposals.downloadZip', $proposal->id) }}" class="btn-sm btn-info" style="white-space: nowrap;">
                            ...
                            <td class="d-flex">
                                        ...
                                        <div class="modal-content shadow-sm">
                                            ...
                                            <div class="modal-footer">
                                                @if ($proposal->status_disposisi == 'Selesai' && $proposal->jenis_proposal === 'Surat Masuk')
                                                    <a href="{{ route('keuangan.proposals.pdf', $proposal->id) }}" target="_blank" class="btn btn-primary">
                                                        <i class="fas fa-print"></i> Cetak PDF
                                                    </a>
                                                    <a href="{{ route('keuangan.proposals.word', $proposal->id) }}" class="btn btn-info" target="_blank">
                                                        <i class="fas fa-file-word"></i> Cetak Word
                                                    </a>
                                                @elseif ($proposal->status_disposisi == 'Selesai' && $proposal->jenis_proposal === 'Surat Tugas')
                                                    <a href="{{ route('keuangan.surat-tugas.pdf', $proposal->id) }}" target="_blank" class="btn btn-primary">
                                                        <i class="fas fa-print"></i> Cetak PDF
                                                    </a>
                                                    <a href="{{ route('keuangan.surat-tugas.word', $proposal->id) }}" class="btn btn-info" target="_blank">
                                                        <i class="fas fa-file-word"></i> Cetak Word
                                                    </a>
                                                @endif
                                ...
                                <!-- Delete Button -->
                                <form action="{{ route('keuangan.arsip.destroy', $proposal->id) }}" method="POST" style="display:inline;">
                                    @csrf
                                    @method('DELETE')
