...
<div class="container pt-4" style="font-family: 'Roboto', sans-serif;">
    <h2 style="font-weight: 700; color: #2C3E50;">Manajemen Kop Surat</h2>
    ...
    <button class="btn btn-primary mb-3 shadow-sm" style="background-color: #3498DB; border: none; border-radius: 50px; padding: 8px 20px;" data-toggle="modal" data-target="#createModal">
        <i class="fas fa-plus-circle"></i> Tambah Kop Surat
    </button>
        ...
        <table class="table table-bordered table-hover shadow-sm" style="background-color: #f8f9fa; border-radius: 8px;">
            ...
            <tbody>
                ...
                <tr class="align-middle">
                    ...
                    <td>{{ $item->nama }}</td>
                    <td><img src="{{ asset($item->kop_surat) }}" height="50"></td>
                    <td><img src="{{ asset($item->footer) }}" height="50"></td>
                    <td class="d-flex justify-content-center">
                        <button class="btn btn-outline-warning btn-sm mr-2" data-toggle="modal" data-target="#editModal{{ $item->id }}">
                            <i class="fas fa-edit"></i>
                        </button>
                        <form action="{{ route('admin.kop-surat.destroy', $item->id) }}" method="POST" onsubmit="return confirm('Yakin ingin menghapus?')">
                            @csrf @method('DELETE')
                ...
                <div class="modal fade" id="editModal{{ $item->id }}" tabindex="-1" role="dialog">
                    <div class="modal-dialog" role="document">
                        <form action="{{ route('admin.kop-surat.update', $item->id) }}" method="POST" enctype="multipart/form-data">
                            @csrf @method('PUT')
...
<div class="modal fade" id="createModal" tabindex="-1" role="dialog">
    <div class="modal-dialog" role="document">
        <form action="{{ route('admin.kop-surat.store') }}" method="POST" enctype="multipart/form-data">
            @csrf
            <div class="modal-content">
                <div class="modal-header" style="background-color: #2C3E50; color: white;">
