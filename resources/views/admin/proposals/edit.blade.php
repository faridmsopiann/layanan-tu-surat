...
<div class="container pb-4">
    ...
    <form action="{{ route('admin.proposals.update', $proposal->id) }}" method="POST">
        ...
        <div class="form-group">
            ...
            <input type="file" class="form-control-file" name="soft_file" accept=".pdf">
            <small class="form-text text-muted">Jika ingin meng-upload file baru, pilih file di bawah. Jika tidak, biarkan kosong.</small>
            @if($proposal->soft_file)
                <div class="mt-2">
                    <strong>File saat ini:</strong> 
                    <a href="{{ Storage::url($proposal->soft_file) }}" target="_blank">{{ basename($proposal->soft_file) }}</a>
                </div>
            @else
                <div class="mt-2">Belum ada file di-upload.</div>
            @endif
        </div>
