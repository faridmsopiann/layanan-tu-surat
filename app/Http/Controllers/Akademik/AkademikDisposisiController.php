...
class AkademikDisposisiController extends Controller
{
    // Menampilkan semua proposal untuk disposisi oleh Dekan
    public function index()
    {
        $proposals = Proposal::where('status_disposisi', 'Menunggu Approval Akademik')
            ->orWhere('tujuan_disposisi', 'Akademik')
            ->paginate(5);
        return view('akademik.disposisi.index', compact('proposals'));
    }
    ...
    public function edit($id)
    {
        ...
        return view('akademik.disposisi.disposisi_form', compact('proposal'));
    }
    ...
    {
        $proposal = Proposal::findOrFail($id);
        return view('akademik.disposisi.reject_form', compact('proposal'));
    }
    ...
    public function updateDisposisi(Request $request, Proposal $proposal)
    {
        ...
        return redirect()->route('akademik.disposisi.index')->with('success', 'Proposal berhasil didisposisi.');
    }
    ...
    public function updateReject(Request $request, Proposal $proposal)
    {
        ...
        $modal->update([
            'tujuan' => 'Akademik',  // Update tujuan dari request
            'status' => 'Ditolak',
            'tanggal_diterima' => $modal->tanggal_diterima,  // Tetap gunakan tanggal_diterima yang sudah ada
            'tanggal_proses' => now(),  // Update dengan tanggal saat ini
            'diverifikasi_oleh' => auth()->user()->name,  // Nama user yang sedang login
        ...
        return redirect()->route('akademik.disposisi.index')->with('success', 'Proposal berhasil ditolak.');
    }
}
