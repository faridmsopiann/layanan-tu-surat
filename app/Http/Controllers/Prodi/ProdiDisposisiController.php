...
class ProdiDisposisiController extends Controller
{
    // Menampilkan semua proposal untuk disposisi oleh Dekan
    public function index()
    {
        $proposals = Proposal::where('status_disposisi', 'Menunggu Approval Prodi')
            ->orWhere('tujuan_disposisi', 'Prodi')
            ->paginate(5);
        return view('prodi.disposisi.index', compact('proposals'));
    }
    ...
    public function edit($id)
    {
        ...
        return view('prodi.disposisi.disposisi_form', compact('proposal'));
    }
    ...
    {
        $proposal = Proposal::findOrFail($id);
        return view('prodi.disposisi.reject_form', compact('proposal'));
    }
    ...
    public function updateDisposisi(Request $request, Proposal $proposal)
    {
        ...
        $modal->update([
            ...
            'diverifikasi_oleh' => 'Dewi Khairani, M.Sc',  // Nama user yang sedang login
        ...
        return redirect()->route('prodi.disposisi.index')->with('success', 'Proposal berhasil didisposisi.');
    }
    ...
    public function updateReject(Request $request, Proposal $proposal)
    {
        ...
        $modal->update([
            'tujuan' => 'Prodi',  // Update tujuan dari request
            'status' => 'Ditolak',
            'tanggal_diterima' => $modal->tanggal_diterima,  // Tetap gunakan tanggal_diterima yang sudah ada
            'tanggal_proses' => now(),  // Update dengan tanggal saat ini
            'diverifikasi_oleh' => 'Dewi Khairani, M.Sc',  // Nama user yang sedang login
        ...
        return redirect()->route('prodi.disposisi.index')->with('success', 'Proposal berhasil ditolak.');
    }
}
