...
class ProdiKimiaDisposisiController extends Controller
{
    // Menampilkan semua proposal untuk disposisi oleh Dekan
    public function index()
    {
        $proposals = Proposal::where('status_disposisi', 'Menunggu Approval Prodi Kimia')
            ->orWhere('tujuan_disposisi', 'Prodi Kimia')
            ->paginate(5);
        return view('prodi-kimia.disposisi.index', compact('proposals'));
    }
    ...
    {
        $proposal = Proposal::findOrFail($id);
        return view('prodi-kimia.disposisi.disposisi_form', compact('proposal'));
    }
    ...
    {
        $proposal = Proposal::findOrFail($id);
        return view('prodi-kimia.disposisi.reject_form', compact('proposal'));
    }
    ...
    public function updateDisposisi(Request $request, Proposal $proposal)
    {
        ...
        return redirect()->route('prodi-kimia.disposisi.index')->with('success', 'Proposal berhasil didisposisi.');
    }
    ...
    public function updateReject(Request $request, Proposal $proposal)
    {
        ...
        $modal->update([
            'tujuan' => 'Prodi Kimia',  // Update tujuan dari request
            'status' => 'Ditolak',
            'tanggal_diterima' => $modal->tanggal_diterima,  // Tetap gunakan tanggal_diterima yang sudah ada
            'tanggal_proses' => now(),  // Update dengan tanggal saat ini
            'diverifikasi_oleh' => auth()->user()->name,  // Nama user yang sedang login
        ...
        return redirect()->route('prodi-kimia.disposisi.index')->with('success', 'Proposal berhasil ditolak.');
    }
