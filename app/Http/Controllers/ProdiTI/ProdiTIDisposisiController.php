...
class ProdiTIDisposisiController extends Controller
{
    // Menampilkan semua proposal untuk disposisi oleh Dekan
    public function index()
    {
        $proposals = Proposal::where('status_disposisi', 'Menunggu Approval Prodi Teknik Informatika')
            ->orWhere('tujuan_disposisi', 'Prodi Teknik Informatika')
            ->paginate(5);
        return view('prodi-ti.disposisi.index', compact('proposals'));
    }
    ...
    {
        $proposal = Proposal::findOrFail($id);
        return view('prodi-ti.disposisi.disposisi_form', compact('proposal'));
    }
    ...
    {
        $proposal = Proposal::findOrFail($id);
        return view('prodi-ti.disposisi.reject_form', compact('proposal'));
    }
    ...
    public function updateDisposisi(Request $request, Proposal $proposal)
    {
        ...
        $modal = ModalDisposisi::where('proposal_id', $proposal->id)
            ->where('tujuan', 'Prodi Teknik Informatika')
        ...
        return redirect()->route('prodi-teknik-informatika.disposisi.index')->with('success', 'Proposal berhasil didisposisi.');
    }
    ...
    public function updateReject(Request $request, Proposal $proposal)
    {
        ...
        $modal->update([
            'tujuan' => 'Prodi Teknik Informatika',  // Update tujuan dari request
            'status' => 'Ditolak',
            'tanggal_diterima' => $modal->tanggal_diterima,  // Tetap gunakan tanggal_diterima yang sudah ada
            'tanggal_proses' => now(),  // Update dengan tanggal saat ini
            'diverifikasi_oleh' => auth()->user()->name,  // Nama user yang sedang login
        ...
        return redirect()->route('prodi-teknik-informatika.disposisi.index')->with('success', 'Proposal berhasil ditolak.');
    }
